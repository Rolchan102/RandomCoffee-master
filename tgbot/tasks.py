...
from tgbot.reg_bot import Registration
from tgbot.quest_bot import Questionnaire

logger = get_task_logger(__name__)

@app.task(name='register_user', ignore_result=True)
def register_user():
    """Регистрация новых пользователей."""

    # Находим новых пользователей
    new_users = Registration.get_new_users()
    for user in new_users:
        Registration(user).register_user()

@app.task(name='send_quest', ignore_result=True)
def send_quest():
    """Расслыка еженедельных опросов."""
    ...
    for user in users:
        telegram = Questionnaire(user)
        poem_text, poem_id = telegram.register_email()
        send_message(
